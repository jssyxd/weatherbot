...
3. tree4 已按运行要求采用 120 秒全量轮询；这短于官方“至少缓存 15 分钟”的建议，因此必须记录 429、响应龄期和首见延迟，并在 429 时遵守 `Retry-After` 或至少 120 秒退避。
...
| 文档页面 | 已确认内容 | tree4 决策 |
|---|---|---|
| Introduction | HTTPS REST、`X-API-Key`、JSON 顶层结构、限额与缓存建议。 | 使用服务端 Header 认证，封装超时、HTTP 状态、JSON 结构校验与 429 退避；运行策略为 120 秒全量轮询。 |
