...
- **死桶**：新日高 12→13°C ⇒ "最高 12°C" 桶 `[12,13)` 明确不可能 ⇒ 买该桶 NO；新日低 12→11°C ⇒ "最低 12°C" 桶 `[12,13)` 明确不可能 ⇒ 买该桶 NO。选择的是**承载前一个极值**的桶（`lo ≤ previous < hi`），跳多度时回退到最近被跨过的边界。
- **价格门**：NO 盘口 ask 严格位于 `(0.05, 0.95)`（5~95 美分）才下注；其余（≤5¢ 或 ≥95¢）不下注。
